...
public class GraphWindow {
	...
	public static void loop() {
		...
		while (!glfwWindowShouldClose(windowID)) {
			...
			GL11.glClear(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT /*| GL11.GL_TRANSFORM_BIT*/);
			...
			for (float x = -10; x < 10; x += 0.5f) {
				for (float y = -10; y < 10; y += 0.5f) {
					...
					GL11.glVertex3f(x, y, (float) Math.tan(x + y));
					GL11.glVertex3f(x, (y + 1), (float) Math.tan(x + (y + 1)));
					GL11.glVertex3f(x, y, (float) Math.tan(x + y));
					GL11.glVertex3f((x + 1), y, (float) Math.tan((x + 1) + y));
				}
			}
